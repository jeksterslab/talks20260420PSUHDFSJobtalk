...
\begin{tikzpicture}[
	...
	\node[coretxt] at ($(core.center)+(0,0.45)$) {person-specific dynamic phenotypes};
	...
	at ($(harm.north west)+(1.18,-0.5)$) {People and contexts};
	\node[smalltxt, anchor=west, align=left]
	at ($(harm.north west)+(1.18,-1.65)$)
	{from within-person dynamics\\to dyads, families, and studies,\\across home, school, work, peers,\\and daily life};
	
	\node[boxtitle, anchor=west, align=left]
	at ($(integrate.north west)+(1.18,-0.5)$) {Linked data streams};
	\node[smalltxt, anchor=west, align=left]
	at ($(integrate.north west)+(1.18,-1.5)$)
	{linking self-report, wearables,\\physiology, passive data,\\and electronic health records};
	
	\node[boxtitle, anchor=west, align=left]
	at ($(models.north west)+(1.18,-0.5)$) {Broader dynamic models};
	\node[smalltxt, anchor=west, align=left]
	at ($(models.north west)+(1.18,-1.5)$)
	{TVEM, latent transition models,\\dyadic models,\\continuous-time models};
	
	\node[boxtitle, anchor=west, align=left]
	at ($(targets.north west)+(1.18,-0.50)$) {Substantive targets};
	\node[smalltxt, anchor=west, align=left]
	at ($(targets.north west)+(1.18,-1.50)$)
	{family stress spillover,\\affective recovery, sleep rhythms,\\and daily regulation};
